...
for i in range(iterations):
    ...
    # Apply mutation to the solutions generated by the ants
    for ant_path, ant_result in optimiser.history.paths_results_history:
        mutation_attempts = 0
        while mutation_attempts < max_mutation_attempts:
            mutated_solution = optimiser.mutate_solution(ant_path)  # Mutate the solution (path)
            mutated_result = prob.evaluate(mutated_solution)
            if not pareto_front_archive.contains(
                    mutated_solution):  # Ensure mutated solution is not already in the Pareto archive
                ...
                break  # Exit mutation loop if a valid mutated solution is found
            else:
                mutation_attempts += 1
